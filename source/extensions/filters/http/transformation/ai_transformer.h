...
#include "api/envoy/config/filter/http/transformation/v2/transformation_filter.pb.h"
...
struct PromptEnrichment {
  ...
  const std::vector<const Message>& prepend() const { return prepend_; }

  const std::vector<const Message>& append() const { return append_; }

private:
  std::vector<const Message> prepend_;
  std::vector<const Message> append_;
...
private:
  ...
  std::vector<const FieldDefault> field_defaults_;
